...
from ..strategies import NFF, HAFF, FMomentum


def build_bot():
    Config(config_dir='configs', filename='admiral.json', reload=True, records_dir='records/admiral/',
           use_ram=True, trailing_stops=True, exit_signals=False, trailing_loss=False, fixed_closer=True, hedging=True)
    ...
    sts = [St(symbol=sym, sessions=Sessions(intl)) for sym in syms for St in [NFF, HAFF, FMomentum]]
