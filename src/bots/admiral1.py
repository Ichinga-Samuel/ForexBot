...
from ..strategies import FingerFractal, FingerTrap, Momentum, RMomentum, NMomentum
...
def build_bot():
    Config(config_dir='configs', filename='admiral_1.json', reload=True, records_dir='records/admiral1/',
           use_ram=True, trailing_stops=True, exit_signals=True, trailing_loss=True, fixed_closer=False)
    ...
    sts = [St(symbol=sym, sessions=Sessions(intl)) for sym in syms for St in [NMomentum, Momentum, RMomentum]]
