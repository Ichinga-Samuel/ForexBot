...
async def trailing_sl(*, position: TradePosition):
    try:
        config = Config()
...
async def modify_stops(*, position: TradePosition, order: dict, extra: float = 0.0, tries: int = 4):
    try:
        ...
        candles.rename(inplace=True, **{'ATRr_14': 'atr'})
        current = candles[-1]
        ...
        if position.type == OrderType.BUY:
            ...
            assert sl < position.sl, f"current_sl={position.sl} <than new {sl=} in short"
            if (position.profit / expected_profit) > extend_profit:
                tp = position.tp - current.atr * atr_multiplier
                change_tp = True
            else:
